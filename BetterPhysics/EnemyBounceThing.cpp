...
void __cdecl EnemyBounceThing_r(Uint8 playerID, float speedX, float speedY, float speedZ)
{
	if (!MechanicsTweaks)
	{
		...
		{
			// what
			v5->VelocityDirection.x = speedX;
			v5->VelocityDirection.y = speedY;
			v5->VelocityDirection.z = speedZ;

			// obvious
			NJS_VECTOR newSpeed = { speedX, speedY, speedZ };

			// what
			WorldToPlayer(data1, &newSpeed);
		...
		float origGrav = PhysicsArray[data1->CharID].Gravity;
		if (PhysicsArray[data1->CharID].Gravity == 0.0f)
		...
		PhysicsArray[data1->CharID].Gravity = origGrav;
